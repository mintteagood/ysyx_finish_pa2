...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw  ,  RR, R(dest) = SEXT(BITS((src1 + src2),31,0),32));
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw ,   RR, R(dest) = SEXT(BITS((src1 << src2), 31, 0), 32));
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and ,    RR, R(dest) = src1 & src2 );
  INSTPAT("??????? ????? ????? 011 ????? 01100 11", sltu,    RR, R(dest) = (unsigned int)src1 < (unsigned int) src2 ? 1:0);
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 01110 11", mulw  ,  RR, R(dest) = SEXT(BITS(src1 * src2,31,0),32));
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw  ,  RR, R(dest) = SEXT((BITS(src1 ,31,0)/BITS(src2,31,0)),32));
//if-else
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw  ,  RR, R(dest) = SEXT((BITS(src1 ,31,0)%BITS(src2,31,0)),32));
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt  ,   B, s->dnpc = (SEXT(src1,32)< SEXT(src2,32)) ? s->pc + dest :  s->pc+4 );
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt ,    RR,  R(dest) = (SEXT(src1,32)< SEXT(src2,32)) ? 1:0 );
//store
  ...
  INSTPAT("0100000 ????? ????? 000 ????? 01110 11", subw ,   RR, R(dest) = SEXT(BITS(src1 - src2,31,0),32) );
//movsx
...
//recursion有问题
//shift有问题
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw ,   RR, R(dest) = SEXT(BITS(BITS(src1,31,0) >> BITS(src2,4,0),31,0),32) );
  INSTPAT("000000? ????? ????? 101 ????? 00110 11", srliw ,  I,  R(dest) = SEXT(BITS(src1,31,0) >> src2,32) );
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw ,   RR, R(dest) = SEXT(BITS(SEXT(src1,32) >> BITS(src2,4,0),31,0),32) );
...
//hello-str
  ...
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu ,   RR, R(dest) = src1 % (unsigned int)src2 );
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu ,   RR, R(dest) = src1 / (unsigned int)src2  );
//coremark  
