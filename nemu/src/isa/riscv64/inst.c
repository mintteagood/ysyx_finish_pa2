...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli ,   I, R(dest) = SEXT((unsigned int)BITS((src1 >> src2), 4, 0),5) ,printf("current pc is %lx ",s->pc),printf("R(10) is:%lx\n",R(10)));
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge  ,   B, s->dnpc = (src1>=src2) ? s->pc + dest :  s->pc+4  ,printf("current pc is %lx ",s->pc),printf("bne next s->dnpc is:%lx\n",s->dnpc),printf("bnesrc1 is:%lx\n",src1),printf("bnesrc2 is:%lx\n",src2),printf("R(10) is:%lx\n",R(10)));
