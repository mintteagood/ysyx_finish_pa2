...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh    ,  S, Mw(src1 + dest, 2, BITS(src2,15,0)),printf("current pc is %lx ",s->pc),printf("shok\n cunru :%lx\n",R(src1 + dest)));
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai ,   I, R(dest) = src1 >> src2 ,printf("current pc is %lx ",s->pc));
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu ,    I, R(dest) = SEXT((unsigned int)BITS(Mr(src1 + src2, 4),31,0),32) ,printf("current pc is %lx ",s->pc));

  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi ,   I,  R(dest) = src1 & src2 ,printf("current pc is %lx ",s->pc));
  ...
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and ,    RR, R(dest) = src1 & src2 , printf("current pc is %lx ",s->pc));
  INSTPAT("??????? ????? ????? 011 ????? 01100 11", sltu,    RR, R(dest) = src1 < ( src2) ? 1:0,printf("current pc is %lx ",s->pc),printf("sltu is:%lx\n",R(dest)),printf("bijiaoshu is:%x\n",(unsigned int) src2 ));
  INSTPAT("0000000 ????? ????? 100 ????? 00100 11", xori ,   I,  R(dest) = src1 ^ src2 , printf("current pc is %lx ",s->pc));
