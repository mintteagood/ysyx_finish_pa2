...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr  ,  I, s->dnpc = (src1 +src2) & ~1, R(dest) = s->pc + 4,printf("auipc next dnpc is:%lx\n",s->dnpc));
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu,   I, R(dest) = src1 < ( src2) ? 1:0,printf("sltiu is:%lx\n",R(dest)),printf("bijiaoshu is:%x\n",(unsigned int) src2 )); //buqueding
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw    ,  I, R(dest) = Mr(src1 + src2, 4),printf("lw R(dest) is:%lx\n",R(dest)));
