...
INSTPAT_START();
  ...
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui   ,  U, R(dest) = src1,printf("R(1) is:%lx\n",R(1)));
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, R(dest) = src1 <<src2,printf("R(1) is:%lx\n",R(1)));
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal,     J, R(dest) = s->pc + 4, s->dnpc = s->pc + src1,printf("current pc is %lx ",s->pc),printf("R(1) is:%lx\n",R(1))); 
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr  ,  I, s->dnpc = (src1 +src2) & ~1, R(dest) = s->pc + 4,printf("R(1) is:%lx\n",R(1)));
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub  ,   RR, R(dest) = src1 - src2,printf("R(1) is:%lx\n",R(1)));
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu,   I, R(dest) = src1 < ( src2) ? 1:0,printf("R(1) is:%lx\n",R(1))); //buqueding
  ...
  INSTPAT("??????? ????? ????? 011 ????? 01100 11", sltu,    RR, R(dest) = (unsigned int)src1 < (unsigned int) src2 ? 1:0,printf("current pc is %lx ",s->pc),printf("sltu is:%lx\n",R(dest)),printf("bijiaoshu is:%x\n",(unsigned int) src2 ),printf("R(10) is:%lx\n",R(10)));
...
//if-else
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt  ,   B, s->dnpc = (SEXT(src1,32)< SEXT(src2,32)) ? s->pc + dest :  s->pc+4  ,printf("current pc is %lx ",s->pc),printf("blt next s->dnpc is:%lx\n",s->dnpc),printf("bltsrc1 is:%lx\n",src1),printf("bltsrc2 is:%lx\n",src2),printf("R(10) is:%lx\n",R(10)));
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt ,    RR,  R(dest) = (SEXT(src1,32)< SEXT(src2,32)) ? 1:0 , printf("current pc is %lx ",s->pc),printf("slt R(dest) is:%lx\n",R(dest)),printf("R(10) is:%lx\n",R(10)));
//store
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh ,     I,  R(dest) = SEXT(BITS(Mr(src1 + src2, 2),15,0),16) ,printf("current pc is %lx ",s->pc),printf("R(10) is:%lx\n",R(10)));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu ,    I,  R(dest) = BITS(Mr(src1 + src2, 2),15,0) ,printf("current pc is %lx ",s->pc),printf("R(10) is:%lx\n",R(10)));
...
//movsx
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw ,  I,  R(dest) = SEXT(BITS(src1,31,0),32)>>src2 ,printf("current pc is %lx ",s->pc),printf("R(10) is:%lx\n",R(10)));
...
//shift有问题
  ...
  INSTPAT("000000? ????? ????? 101 ????? 00110 11", srliw ,  I,  R(dest) = SEXT(BITS(src1,31,0) >> src2,32) ,printf("current pc is %lx ",s->pc),printf("R(10) is:%lx\n",R(10)));
