...
static void decode_operand(Decode *s, word_t *dest, word_t *src1, word_t *src2, int type) {
  ...
  switch (type) {
    ...
    case TYPE_B: src1I(rs1); src2R(rs2); destI(immB(i)); break;
  }
}
...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal,     J, R(dest) = s->pc + 4, s->dnpc = s->pc + src1,printf("next dnpc is:%lx\n",s->dnpc)); 
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(dest) = src1 + s->pc,printf("auipcok\n"));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr  ,  I, R(dest) = s->pc + 4,s->dnpc = (src1 +src2) & ~1,printf("next dnpc is:%lx\n",s->dnpc));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq  ,   B, s->dnpc = (src1==src2) ? s->pc + dest :  s->pc + 0 ,printf("beq next s->dnpc is:%lx\n",s->dnpc),printf("beqsrc1 is:%lx\n",src1),printf("beqsrc2 is:%lx\n",src2));
